...
PointLightSystem::~PointLightSystem() { vkDestroyPipelineLayout(lveDevice.getDevice(), pipelineLayout, nullptr); }
...
void PointLightSystem::update(Rendering::FrameInfo &frameInfo, Rendering::GlobalUbo &ubo)
{
...
void PointLightSystem::render(Rendering::FrameInfo &frameInfo)
{
    // sort lights
    std::map<float, Rendering::GameObject::id_t> sorted;
    for (auto &kv : frameInfo.gameObjects) {
        auto &obj = kv.second;
        if (obj.pointLight == nullptr)
            continue;
