...
ApeModel::ApeModel(ApeDevice &apeDevice, const std::vector<Vertex> &vertices) : apeDevice{apeDevice}
{
...
void ApeModel::createVertexBuffers(const std::vector<Vertex> &vertices)
{
    ...
    void *data;
    vkMapMemory(apeDevice.device(), vertexBufferMemory, 0, bufferSize, 0, &data);
    memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
    vkUnmapMemory(apeDevice.device(), vertexBufferMemory);
}

void ApeModel::draw(VkCommandBuffer commandBuffer) { vkCmdDraw(commandBuffer, vertexCount, 1, 0, 0); }
